...
import InfinitScroller from '@components/infinitscroller/infinitscroller'
...
interface IDownloadHistoryStates {
   init: boolean,
   pageIndex: number,
   hasData: boolean,
   fetchingMore: boolean
   datas: IDownloadPackage[],
}
...
const DownloadsHistory = () => {
   ...
   const [states, setStates] = useState<IDownloadHistoryStates>({
      datas: [],
      init: false,
      pageIndex: 1,
      hasData: false,
   ...
   useEffect(() => {
      changeStates(['init'], [true])
   }, [state, setPackageDownloadStore])
   ...
   const loadMore = async () => {
      changeStates(['fetchingMore'], [true])
      //
      if (timeoutRef.current) {
         clearTimeout(timeoutRef.current)
      }
      //
      let _numberPerItems = 3,
         _startIndex = (states.pageIndex - 1) * _numberPerItems,
         _endIndex = _startIndex + _numberPerItems
      //
      timeoutRef.current = setTimeout(async () => {
         const newDatas = await (await Promise.resolve<IDownloadPackage[]>(contents))
            .slice(_startIndex, _endIndex)
         const _hasData = newDatas.length > 0;
         setStates(prevStates => ({
            ...prevStates,
            hasData: _hasData,
            fetchingMore: false,
            pageIndex: prevStates.pageIndex + 1,
            datas: [...prevStates.datas, ...newDatas],
         }))
      }, Math.random() * 1000 + 250);
   }
   /** DOWNLOAD ITEM TEMPLATE */
   const DownloadTemplate = (item: IDownloadPackage) => {
      return <Panel key={item.downloadKey} className='w-full my-1 rounded-md text-sm'
         ...
         {
            (!item.status.isCompleted && item.status.isStarted) && (
   ...
   const LoadingTemplate = () => {
      return (
         <div className='flex items-center justify-center w-full my-2'>
            <ProgressSpinner className='place-self-center' />
         </div>
      )
   }
   ...
   return (
      <div className='flex flex-col justify-between overflow-hidden max-h-[560px]'>
         ...
         <div className='flex-1 justify-items-center my-2 overflow-y-auto max-h-[450px]' ref={virtualContainerRef}>
            {/* DOWNLOAD LIST */}
            {
               state === 'loading' ? <LoadingTemplate /> :
                  state === 'hasError' ? <p className='text-center my-2 text-sm'>Sorry something is amiss, an error occured, can't get download list.</p> :
                     state === 'hasValue' && (
                        states.init && (
                           <InfinitScroller loadMore={states.hasData} loadOnMount={true} container={virtualContainerRef?.current} callBack={loadMore}>
                              {
                                 states.datas?.length > 0 ?
                                    (
                                       states.datas?.filter((filter: IDownloadPackage) => statusFilter(filter)).map((item: IDownloadPackage) => DownloadTemplate(item))
                                    ) : (
                              ...
                              {states.fetchingMore && <LoadingTemplate />}
                           </InfinitScroller >
                        )
                     )
            }
