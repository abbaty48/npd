...
export class PackageDownloader implements IDownloadItem {
  ...
  private _sleep = (callback: () => void, durationMS = 1000) =>
    new Promise((resolve) => {
      setTimeout(() => {
        callback();
        resolve(undefined);
  ...
  public downloadPackageOnly = (
    ...
    try {
      ...
      this._response.on(
        'response',
        (resp: { headers: { [x: string]: string } }) => {
          totalBytes = parseInt(resp.headers['content-length']);
          this.onChangeStatus(['isStarted'], [true], onStatus);
        }
      );
      //
      this._response.on('data', (chunk: string | any[]) => {
        ...
        //
        this.onChangeStatus(['isDownloading'], [true], onStatus);
        //
        onProgress(Math.floor(Math.round((receiveBytes * 100) / totalBytes)));
      });
      //
      this._response.on('complete', () => {
        this.onChangeStatus(
          ['isStarted', 'isCompleted', 'isDownloading'],
          [false, true, false],
          onStatus
        );
        onCompleted();
      ...
      //
      this._response.on('error', (e: Error) => {
        this.onChangeStatus(
          [
            'isFailed',
            'isStarted',
            ...
            this._response.on('response', (resp) => {
              ...
              resp.on('data', (chunk) => {
                ...
                this.onChangeStatus(
                  ...
                  [true, `Downloading ${this.originalFileName}`],
                  onStatus
                );
                // update recieve bytes
                receiveBytes += chunk.length;
